...
/**
 * Client wrapper for Bulkpe APIs. Provides a WebClient with authentication and resilience (circuit
 * breaker).
 */
@Slf4j
@Service
public class BulkpeApi {
  ...
  /**
   * Constructor injection ensures immutability and easier testing. CircuitBreakerRegistry provides
   * a centralized circuit breaker configuration.
   */
  public BulkpeApi(
  ...
  /**
   * Builds a WebClient instance pre-configured with: - Base URL for Bulkpe API - Authorization
   * header - Circuit breaker filter
   */
  ...
  /**
   * Wraps each WebClient request with Resilience4j circuit breaker. If calls repeatedly fail, the
   * breaker will open and short-circuit further requests.
   *
  ...
  private ExchangeFilterFunction circuitBreakerFilter(CircuitBreaker circuitBreaker) {
    return (request, next) -> {
      ...
      return next.exchange(request)
          ...
          .doOnSuccess(
              response -> log.info("Bulkpe API responded with status {}", response.statusCode()))
          .doOnError(ex -> log.error("Bulkpe API request failed: {}", ex.getMessage(), ex))
          // Optional fallback (currently just rethrows a wrapped exception)
          .onErrorResume(
              ex ->
                  Mono.error(new RuntimeException("Circuit breaker triggered for Bulkpe API", ex)));
    };
  }
}
