 ...
 * Provides endpoints for CRUD operations on goals and related operations
 * 
...
public class GoalController {
    ...
    public ResponseEntity<Map<String, List<GoalDTO>>> createGoal(
            @RequestBody Map<String, List<GoalDTO>> requestBody) {
        
        log.info("POST /api/v1/goals - Creating new goal");
        
        try {
            List<GoalDTO> goalData = requestBody.get("data");
     ...
     * 
     * @param goalId The ID of the goal
     * @param requestBody Request body containing order data
     * @return ResponseEntity containing the created orders
     */
    @PostMapping("/{goalId}/orders")
    @Operation(
        summary = "Create orders for a goal",
        description = "Creates new orders associated with a specific investment goal"
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "201",
    ...
    public ResponseEntity<Map<String, List<OrderDTO>>> createGoalOrders(
            @Parameter(description = "Goal ID", required = true)
            @PathVariable String goalId,
            @RequestBody Map<String, List<OrderDTO>> requestBody) {
        
        log.info("POST /api/v1/goals/{}/orders - Creating orders for goal", goalId);
        
        try {
            List<OrderDTO> orderData = requestBody.get("data");
            if (orderData == null || orderData.isEmpty()) {
                log.warn("Invalid request body: missing or empty data array");
                return ResponseEntity.badRequest().build();
            }
            
            List<OrderDTO> createdOrders = orderService.createOrdersForGoal(goalId, orderData);
            log.info("Successfully created {} orders for goal {}", createdOrders.size(), goalId);
            
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(Map.of("data", createdOrders));
                    
        } catch (Exception e) {
            log.error("Error creating orders for goal {}: {}", goalId, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
}
