...
const phaseConfig: Record<DisplayPhase, {
  ...
  completed: {
    ...
    badgeText: "Done ✓", badgeColor: "#16A34A",
    showSpinner: false,
  },
  failed: {
    icon: "close-circle",
...
export function OneTimePurchaseCard({ payment }: OneTimePurchaseCardProps) {
  const status: DisplayPhase = payment?.buy_status ?? "loading";
    ...
    <View style={[styles.card, status === "not_available" && styles.dimmed]}>
      <View style={styles.row}>
        ...
        <View style={styles.info}>
          ...
          <Text style={styles.subtitle}>One-time lumpsum payment</Text>
        </View>
